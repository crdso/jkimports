...
export default function CartDrawer(){
  ...
  const checkout=()=>{
    ...
    const lines=items.map(i=>`- ${i.quantity}x ${i.name} (${i.variantLabel}) - ${fmt(i.price*i.quantity)}`).join('\n')
    const msg=`Olá, JK IMPORTS! Quero finalizar minha compra:\n\n${lines}\n\n*Total: ${fmt(subtotal)}*`
    const url=`https://api.whatsapp.com/send/?phone=5599984599773&text=${encodeURIComponent(msg)}`
    ...
    <div className={`cart-overlay ${open?'is-open':''}`} onClick={closeCart} aria-hidden={!open}>
      <div className="cart-panel" role="dialog" aria-modal="true" aria-label="Carrinho" onClick={e=>e.stopPropagation()}>
            ...
            <div key={i.variantId} className="cart-item">
              ...
              <div style={{minWidth:0}}>
                ...
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:8}}>
                  ...
                  <button onClick={()=>removeItem(i.variantId)} style={{fontSize:12,color:'var(--site-orange)',background:'none',border:'none',cursor:'pointer',fontWeight:700}}>Remover</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{padding:16,borderTop:'1px solid var(--site-border)',display:'grid',gap:10}}>
          ...
          <p style={{fontSize:11,color:'var(--site-muted)',textAlign:'center'}}>Você será direcionado ao WhatsApp com a lista de itens.</p>
        </div>
      </div>
    </div>
  )
