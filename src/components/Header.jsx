    ...
    <header className={`site-header ${scrolled ? 'site-header-scrolled' : ''} ${menuOpen ? 'site-header-menu-open' : ''}`}>
      <div className="site-header-inner">
        <a href="/" onClick={handleInicio} className="nav-brand">
          <img src={logo} alt="JK IMPORTS" className="nav-brand-logo" />
        ...
        <div className="site-nav-actions hidden lg:flex">
          ...
          <a href="https://api.whatsapp.com/send/?phone=5599984599773" target="_blank" rel="noopener noreferrer" className="nav-store-cta nav-store-cta--whatsapp desktop-only" aria-label="WhatsApp JK IMPORTS">
      ...
      {menuOpen && (
        <div style={{pointerEvents:'auto',position:'absolute',top:'100%',left:0,right:0,background:'var(--site-panel)',borderTop:'1px solid var(--site-hairline)',padding:'12px 16px 16px',display:'grid',gap:10}} className="lg:hidden">
          {nav.map(i => {
            if(i.to==='/') return <a key={i.label} href="/" onClick={handleInicio} style={{padding:'12px 14px',borderRadius:12,background: location.pathname==='/'?'var(--site-orange)':'var(--site-panel-soft)',color: location.pathname==='/'?'#fff':'var(--site-text)',border:'1px solid var(--site-border)',fontWeight:700,textAlign:'center'}}>{i.label}</a>
            return <NavLink key={i.label} to={i.to} style={({isActive})=>({padding:'12px 14px',borderRadius:12,background:isActive?'var(--site-orange)':'var(--site-panel-soft)',border:'1px solid var(--site-border)',fontWeight:700,textAlign:'center',color:isActive?'#fff':'var(--site-text)',display:'block',textDecoration:'none'})}>{i.label}</NavLink>
          })}
